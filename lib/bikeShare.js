...
(function() {
  var async, config, db, feeds, filterCity, getCities, getCity, host, mongo, open, port, prepareResources, remapData, remapErrors, remapLatLong, remapProperty, run, saveAllStations, server, translateData, update, updateData, updateFeeds, updateStations, utils, _ref,
  ...
  updateData = function(next) {
    return async.series([
      ...
      return update(db, 'logs', log, log, function(errors, results) {
        var message;
        if (errors) {
          return next(err);
        }
        ...
        console.log(message);
        return setTimeout(updateData, 120000);
      });
    });
  };
  ...
  module.exports = {
    ...
    closestStations: function(req, res) {
      var cities, city, err, feed, lat, long, query, stations, _ref1, _ref2;
