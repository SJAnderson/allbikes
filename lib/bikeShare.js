...
(function() {
  ...
  remapLatLong = function(station) {
    ...
    latitude = parseFloat(station.lat || station.latitude || station.Latitude || station.la);
    longitude = parseFloat(station.long || station.longitude || station.Longitude || station.lo);
    station.location = {
      type: 'Point',
  ...
  remapProperty = function(station, primary, secondary) {
    ...
    while (secondary < secondary.length) {
      if (station[secondary[a]]) {
        if (primary === 'stationName') {
          station[primary] = station[secondary[a]];
        } else {
          station[primary] = parseInt(station[secondary[a]] || 0);
  ...
  remapData = function(station, done) {
    ...
    station = remapProperty(station, 'nbBikes', ['availableBikes', 'Bikes', 'ba']);
    station = remapProperty(station, 'nbEmptyDocks', ['availableDocks', 'Dockings', 'da']);
    station = remapProperty(station, 'stationName', ['name', 'stationName', 's', 'Address']);
